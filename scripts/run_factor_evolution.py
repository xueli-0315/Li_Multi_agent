...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run hybrid expression/subset GA factor evolution.")
    ...
    parser.add_argument("--ga-mode", choices=["hybrid", "expression", "subset"], default="hybrid")
    ...
    parser.add_argument("--final-audit-top-n", type=int, default=5)
...
def config_from_args(args: argparse.Namespace) -> EvolutionConfig:
    config = EvolutionConfig(
        ...
        final_audit_top_n=args.final_audit_top_n,
    ...
    for attr in ["panel_data_path", "seed_library_path", "factor_library_path", "log_root"]:
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"Final audited: {result.final_audited_count}, failed audit: {result.failed_audit_count}")
    if result.reason:
