    ...
    def evaluate_with_qlib(
        ...
        *,
        ...
        if evaluated_factors == 0:
            self._set_error("no_resolved_signal_columns")
            return None
...
class BacktestRunnerAgent(BaseAgent):
    ...
    def evaluate_with_llm_factors(
        ...
        *,
        ...
        """Qlib LightGBM 主路径：将 LLM 生成的因子写入 parquet，
        用 NestedDataLoader + LGBModel 训练 + 回测。"""
        ...
        # 写入 combined_factors_df.parquet（Qlib StaticDataLoader 格式）
        try:
            qlib_factors_path = self._write_qlib_factor_parquet(factor_values, panel)
        except Exception as exc:
            self.engine._set_error(f"qlib_factor_write_failed:{exc}")
            return None

        # 动态更新 config 中的 StaticDataLoader 路径
        try:
            self._patch_qlib_config_for_factors(qlib_task_config, qlib_factors_path, list(factor_values.columns))
        except Exception as exc:
            self.engine._set_error(f"qlib_config_patch_failed:{exc}")
            return None

        # 运行 Qlib LightGBM 训练 + 回测
        try:
            backtest_report, metrics = self._run_qlib_lightgbm_backtest(
            ...
            backtest_report["qlib_factors_path"] = str(qlib_factors_path)
    ...
    def _write_qlib_factor_parquet(
        ...
        out_path = Path("./combined_factors_df.parquet")
        ...
        for loader in loader_list:
            ...
            if "StaticDataLoader" in loader_class:
                ...
                if isinstance(loader_kwargs, dict):
                    # 动态构建 feature config
                    features = [[], []]  # [expressions, names]
                    for col in factor_columns:
                        features[0].append(col)
                        features[1].append(col)
                    loader_kwargs["config"] = {
                        "feature": features,
                    }
                    loader_kwargs["config_file"] = str(parquet_path)
                    break
    ...
    def run(
        ...
        *,
        ...
        _P("/tmp/backtest_debug.txt").write_text(f"panel_data_path={panel_data_path!r}, qlib_enabled={self.qlib_enabled}")

        # 主路径：ResearchPipeline 预处理 → Qlib LightGBM 训练 + 回测
        ...
        # Qlib 回退路径（使用旧的 evaluate_with_qlib）
        if panel_data_path and self.qlib_enabled:
        ...
        backtest_report = {
            ...
            "cache_mode": "realtime_compute",
            "panel_data_path": panel_data_path,
            "error": self.engine.last_error or "qlib_backtest_failed",
        }
        experiment.backtest_report = dict(backtest_report)
        experiment.metrics = {str(k): float(v) for k, v in metrics.items()}
        experiment.update_timestamps()
        private_context.payload["last_backtest_mode"] = "qlib_local_failed"
        private_context.payload["latest_backtest_report"] = backtest_report
        private_context.payload["latest_metrics"] = metrics
        private_context.payload["latest_experiment"] = experiment.to_dict()
        return AgentResult(
